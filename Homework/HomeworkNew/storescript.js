...
async function getResponce() {
    ...
    for (key in content_price) {
                ...
                <p class="card-text" > Купить <input class="w-50" type="checkbox" name="check" value="0" onClick='this.value = this.checked ? 1 : 0'></p>
                </li>
                        `
            }
...
async function getResponce1() {
    ...
    for (key in content_title) {
                ...
                <p class="card-text">${content[key].description}. Цена ${content[key].price} р.</p>
                <input type="hidden" name= "vendor_code" value=${content[key].vendor_code}>
                <p class="card-text" > Купить <input class="w-25" type="checkbox" name="check" value="0" onClick='this.value = this.checked ? 1 : 0'></p>
                </li>
                        `
            }
